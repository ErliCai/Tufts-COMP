...
from weight_player import TileWeightPlayer
...
if __name__ == '__main__':
    p2 = TileWeightPlayer([[100, -25, 10, 10, -25, 100],
                           [-25, -25, 5, 5, -25, -25],
                           [10, 5, 1, 1, 5, 10],
                           [10, 5, 1, 1, 5, 10],
                           [-25, -25, 5, 5, -25, -25],
    ...
    p1 = []
    total_accumulative_result = np.array([0] * (num_of_iterations))
    for n in range(num_of_players):
        accumulative_score = np.array([0] * (num_of_iterations))
        p1.append(MonteCarloPlayer())
        for i in range(num_of_iterations):
            r = ReversiState(6)
            moves = []
            while not r.is_terminal():
                r = p1[n].act_from_state(r)
                moves.append(r._board)
                r = p2.act_from_state(r)
            score = r.get_score()
            accumulative_score[i] = score[0]+accumulative_score[i-1]
            p1[n].update(moves, score[0])
        total_accumulative_result = total_accumulative_result + accumulative_score
    total_accumulative_result = total_accumulative_result/num_of_players
    for i in range(num_of_iterations):
        total_accumulative_result[i] = total_accumulative_result[i]/(i+1)




    p3 = TileWeightPlayer([[0] * 6] * 6)
    total_accumulative_result2 = np.array([0] * (num_of_iterations))
    for n in range(num_of_players):
        accumulative_score = np.array([0] * (num_of_iterations))
        for i in range(num_of_iterations):
            r = ReversiState(6)
            while not r.is_terminal():
                r = p3.act_from_state(r)
                r = p2.act_from_state(r)
            score = r.get_score()
            accumulative_score[i] = score[0] + accumulative_score[i - 1]
        total_accumulative_result2 = total_accumulative_result2 + accumulative_score
    total_accumulative_result2 = total_accumulative_result2 / num_of_players
    for i in range(num_of_iterations):
        total_accumulative_result2[i] = total_accumulative_result2[i] / (i + 1)

    plt.plot(total_accumulative_result,label='Monte-Carlo Player')
    plt.plot(total_accumulative_result2,label='Randomized Player')
